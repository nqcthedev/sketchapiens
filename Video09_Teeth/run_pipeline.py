# -*- coding: utf-8 -*-
"""
Video09 pipeline: per-shot TTS (Josh) -> ghép ảnh khớp tiếng -> why-are-your-teeth-worse-than-a-cavemans.mp4
Key đọc từ ~/.config/ghepvideo/elevenlabs.key lúc chạy (KHÔNG lưu trong file).
Dùng:
  python3 run_pipeline.py tts [N]   # N = giới hạn test (bỏ trống = full 140)
  python3 run_pipeline.py assemble  # ghép ảnh + audio -> mp4
  python3 run_pipeline.py all       # tts full rồi ghép
Ảnh đặt trong images/ đặt tên 001.png..140.png theo đúng thứ tự shot.
"""
import os, sys, json, time, subprocess, tempfile, urllib.request, urllib.error

HERE   = os.path.dirname(os.path.abspath(__file__))
NARR   = os.path.join(HERE, "Script_Video09_narration.txt")
IMGDIR = os.path.join(HERE, "images")
AUDDIR = os.path.join(HERE, "audio")
OUT    = os.path.join(HERE, "why-are-your-teeth-worse-than-a-cavemans.mp4")
