...
KINDS = [s[1] for s in SHOTS]
sc = [b for i, (n, b) in enumerate(blocks) if "recurring caveman" in b and KINDS[i] != "SCENE_W"]
chk("khối người cổ đại NAM lặp Y NGUYÊN", sc and all(ANC in b for b in sc), f"{len(sc)} khung")
sw = [b for i, (n, b) in enumerate(blocks) if KINDS[i] == "SCENE_W"]
import re as _re
_wb = [_re.search(r"recurring caveman:.{0,600}", b).group(0) for b in sw] if sw else []
chk("khối người cổ đại NỮ lặp Y NGUYÊN", (not sw) or len(set(_wb)) == 1, f"{len(sw)} khung")
...
# ── chữ trên hình ─────────────────────────────────────────────────────────
sp = [n for (n, b), s in zip(blocks, SHOTS) for (l, k, su, t, bg) in [s[:5]]
...
smalllab = [(n, m) for (n, b), s in zip(blocks, SHOTS) for (l, k, su, t, bg) in [s[:5]]
