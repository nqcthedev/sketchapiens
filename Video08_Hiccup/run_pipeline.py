# -*- coding: utf-8 -*-
"""
Video08 pipeline: per-shot TTS (Josh) -> ghép ảnh khớp tiếng -> why-do-you-hiccup.mp4
Key đọc từ ~/.config/ghepvideo/elevenlabs.key lúc chạy (KHÔNG lưu trong file).
Dùng:
  python3 run_pipeline.py tts [N]   # N = giới hạn test (bỏ trống = full 153)
...
NARR   = os.path.join(HERE, "Script_Video08_narration.txt")
IMGDIR = os.path.join(HERE, "images")
AUDDIR = os.path.join(HERE, "audio")
OUT    = os.path.join(HERE, "why-do-you-hiccup.mp4")
...
if __name__ == "__main__":
    ...
    if cmd == "tts":
        ...
        got = tts(0)
        if got >= 153: assemble()
        else: print(f"⚠ Mới có {got}/153 file tiếng, chạy lại 'tts' cho đủ rồi 'assemble'.", flush=True)
