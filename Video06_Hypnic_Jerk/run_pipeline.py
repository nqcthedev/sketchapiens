# -*- coding: utf-8 -*-
"""
Video06 pipeline: per-shot TTS (Josh) -> ghép ảnh khớp tiếng -> final_video.mp4
...
NARR   = os.path.join(HERE, "Script_Video06_narration.txt")
IMGDIR = os.path.join(HERE, "images")
AUDDIR = os.path.join(HERE, "audio")
OUT    = os.path.join(HERE, "why-do-you-jerk-awake.mp4")
...
if __name__ == "__main__":
    ...
    if cmd == "tts":
        ...
        got = tts(0)
        if got >= 155: assemble()
        else: print(f"⚠ Mới có {got}/155 file tiếng, chạy lại 'tts' cho đủ rồi 'assemble'.", flush=True)
