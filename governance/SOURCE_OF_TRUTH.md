...
| **Ba ràng buộc cứng của lời đọc — `!` = 0 · không gạch ngang giữa câu · mỗi câu một dòng** | **`.claude/rules/script-files.md`** | — | **Canonical khai ở `05B-C`.** Rule này có `paths:` nên tự nạp khi đụng file kịch bản. Máy thực thi: `.claude/skills/sketchapiens-bien-tap/qa_kichban.py` — tool **phải** giữ bản sao để chạy độc lập. Năm consumer khác *(`audit-script` · `apply-review` · Writer `SKILL.md` + `CONTRACT.md`)* nhắc lại để tiện đọc, nhưng **không** phải nguồn chuẩn; lệch thì `script-files.md` thắng. ⛔ `I ≈ 0` **đã gỡ 07/08**, không phải ràng buộc thứ tư. Ba consumer từng cùng ghi "4 ràng buộc cứng" — xem `kho/1_luat/RUBRIC_KichBan.md` §80-84. |
